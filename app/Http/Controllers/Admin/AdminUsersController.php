    ...
    public function index(IndexAdminUser $request)
{
    $data = AdminListing::create(AdminUser::class)
        ->processRequestAndGet(
            $request,
            ['id', 'first_name', 'last_name', 'email', 'activated', 'forbidden', 'language', 'last_login_at', 'sexe_id'],
            ['id', 'first_name', 'last_name', 'email', 'language', 'sexe_id'],
            function($query) {
                $query->with('sexe');  // Eager load the sexe relationship
            }
        );
